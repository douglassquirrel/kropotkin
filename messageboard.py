...
def start_consuming(channel, name, key, callback):
    ...
    def dispatch_message(channel, method, properties, body):
        if method.routing_key == key:
            ...
            post(channel, key="process_stopped", body=key)
        else:
            post(channel, key="unknown_message", body=json.dumps({"key": method.routing_key, "body": body}))
    
    queue_name = bind(channel, key, stop_key)

    channel.basic_consume(dispatch_message, queue=queue_name, no_ack=True)
    post(channel, key="process_ready", body=name)
...
def post(channel, key, body=None):
