...
def no_collect_response(connection, message_key):
    ...
    messageboard.bind(connection=connection, key=done_key)
...
def collect_response(connection, message_key, response_key):
    ...
    key1, body1 = messageboard.get_one_message(connection=connection)
    key2, body2 = messageboard.get_one_message(connection=connection)
...
def collect_one_message():
    ...
    return collect_response(connection=connection, message_key='c1m_0', response_key='c1m_test_messages_received')

def collect_two_messages():
    connection = messageboard.get_connection()
    register_messages(connection=connection, prefix='c2m', number=2)
    return no_collect_response(connection=connection, message_key='c2m_0') \
       and collect_response(connection=connection, message_key='c2m_1', response_key='c2m_test_messages_received')
...
def collect_interleaved():
    ...
    return no_collect_response(connection=connection, message_key='ci1_0') \
       and no_collect_response(connection=connection, message_key='ci2_0') \
       and collect_response(connection=connection, message_key='ci1_1', response_key='ci1_test_messages_received') \
       and collect_response(connection=connection, message_key='ci2_1', response_key='ci2_test_messages_received') \

def collect_only_once():
    connection = messageboard.get_connection()
    register_messages(connection=connection, prefix='c1x', number=1)
    return collect_response(connection=connection, message_key='c1x_0', response_key='c1x_test_messages_received') \
       and no_collect_response(connection=connection, message_key='c1x_0')

def collect_twice():
    connection = messageboard.get_connection()
    register_messages(connection=connection, prefix='c2x', number=1)
    if False == collect_response(connection=connection, message_key='c2x_0', response_key='c2x_test_messages_received'):
        return False
    register_messages(connection=connection, prefix='c2x', number=1)
    return collect_response(connection=connection, message_key='c2x_0', response_key='c2x_test_messages_received')
